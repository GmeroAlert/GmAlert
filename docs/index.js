var GmAlert = (function () {
  ...
  function GmInformation(props) {
    ...
    $wrapper.innerHTML = `<div class="${styles$2['infomation-header']}"><div class="${styles$2['infomation-status']}" style="background: ${color};"></div><div style="margin-left: .5em;">${props.headerLeft || '你有一条消息'}</div><div>${props.headerRight || ''}</div></div><div class="${styles$2['infomation-content']}">${props.content}</div>`;
    const $root = getRoot(2);
    const open = () => {
      $root.append($wrapper);
      return new Promise(resolve => {
    ...
    const close = () => {
      return new Promise(resolve => {
        changeAnimation($wrapper, styles$2['infomation-move-out']);
        const handle = e => {
  ...
  function GmMessage(props) {
    ...
    const close = () => {
      ...
      changeAnimation($main, styles$1['message-out']);
      return new Promise(resolve => {
        const handle = e => {
  ...
  class Msg {
    ...
    judgeReMsg(content, type, conf) {
      ...
      if (this.form < 2) {
        this.activeInsts.size >= this.maxCount && this.activeInsts.values().next().value.close();
      } else {
        // 关闭所有消息
